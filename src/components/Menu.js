...
export default function Menu() {
  ...
  const handleDeleteBoard = () => {
    axios.delete(`${process.env.REACT_APP_API_URL}/boards/deleteBoard?boardId=${selectedBoardId}`, { withCredentials: true }).then(res => {
      if (res.status === 200) {
        dispatch(getBoards())
        dispatch(triggerToast({ open: true, severity: "success", message: res.data.message }))
      }
    })
  }
  return (
    <div style={{ position: "fixed", top: "10%", right: "3%", padding: "20px", width: "200px", background: "black" }}>
      <span style={{ display: 'block' }} onClick={handleLogout}>Logout</span>
      <span style={{ display: 'block' }} onClick={() => handleDeleteBoard()}>Delete Board</span>
    </div>
  )
}
